...
import { getHashedPassword, DEFAULT_PASSWORD } from './utils/password';
import { uuid, randomItem, randomItems, randomInt, randomBool, randomImageUrl, randomRating, pastDate, futureDate, createProgressLogger } from './utils/helpers';
import { ALL_USERS, ADMIN_USERS, COMPANY_OWNER_USERS, TOUR_AGENT_USERS, GUIDE_USERS, DRIVER_USERS, TRAVELER_USERS, MULTI_ROLE_USERS } from './data/users';
import { COMPANIES } from './data/companies';
import { GUIDE_PROFILES, MULTI_ROLE_GUIDE_PROFILES, ALL_GUIDE_PROFILES } from './data/guides';
import { DRIVER_PROFILES, MULTI_ROLE_DRIVER_PROFILES, ALL_DRIVER_PROFILES } from './data/drivers';
import { ALL_TOURS, COMPANY_TOURS, INDIVIDUAL_TOURS } from './data/tours';
import { getRandomComment } from './data/reviews';
import { INQUIRY_MESSAGES, RESPONSE_MESSAGES, FOLLOW_UP_MESSAGES, CASUAL_CHAT_MESSAGES, generateConversation } from './data/messages';
...
async function seedReviews(): Promise<void> {
  ...
  const travelerEmails = TRAVELER_USERS.map(u => u.email);
  let reviewCount = 0;
...
async function seedChats(): Promise<void> {
  ...
  const travelerEmails = TRAVELER_USERS.map(u => u.email);
  const guideEmails = GUIDE_USERS.map(u => u.email);
  const companyEmails = COMPANY_OWNER_USERS.map(u => u.email);
