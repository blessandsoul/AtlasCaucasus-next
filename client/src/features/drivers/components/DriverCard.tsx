...
export const DriverCard = ({ driver, className, isFavorited, onFavorite, isCompareSelected, onCompareToggle }: DriverCardProps): JSX.Element | null => {
    ...
    const getPhotoUrl = (): string => {
        if (driver.avatarUrl) return getMediaUrl(driver.avatarUrl);
        if (driver.photos && driver.photos.length > 0) return getMediaUrl(driver.photos[0].url);
        if (driver.photoUrl) return getMediaUrl(driver.photoUrl);
                        ...
                        <>
                            ...
                            {driver.reviewCount > 0 && (
                                <span className="text-sm text-muted-foreground">
                                    &middot; {driver.reviewCount} {driver.reviewCount === 1 ? 'review' : 'reviews'}
                                </span>
                            )}
                        </>
                    ) : (
