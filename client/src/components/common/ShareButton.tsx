...
export function ShareButton({
  ...
  const handleFacebook = useCallback((): void => {
  ...
  const handleTwitter = useCallback((): void => {
    const encodedUrl = encodeURIComponent(fullUrl);
  ...
  const handleClick = useCallback(async (): Promise<void> => {
    ...
    if (shared) return;
    // If native share not available, the dropdown will handle it
  }, [handleNativeShare]);
  ...
  if (supportsNativeShare) {
    return (
      ...
      >
        ...
        {variant === 'default' && <span className="text-sm">{t('share.label')}</span>}
      </Button>
    );
  }
