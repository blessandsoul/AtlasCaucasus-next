...
function ExploreToursContent(): React.ReactElement {
    ...
    const tourIds = useMemo(
        () => data?.items.map((tour) => tour.id) ?? [],
        [data?.items]
    );
    const { data: favoritedSet } = useFavoriteCheck('TOUR', tourIds);
                            ...
                            <div className="grid grid-cols-1 min-[500px]:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {data.items.map((tour) => (
                                    <TourCard
                                        key={tour.id}
                                        tour={tour}
                                        isFavorited={favoritedSet?.has(tour.id) ?? false}
...
function ExploreToursLoading(): React.ReactElement {
    return (
...
export function ExploreToursClient(): React.ReactElement {
    return (
